...
An earlier submission of this work to *JACC: Advances* contained two incorrect
...
| Table 3, sensitivity analyses | `06_edi_sensitivity_analysis.py` | `output/supplementary_data/EDI_Regression_Results.txt` |
| Table 4, external validation | `09_validated_index_sdi.py` | `output/jacr_revision/validated_index_results.json` |
| Figure 1A, choropleth | `04_choropleth_map.py` | `output/figures/` |
| Figure 1B, forest plot | `10_jacr_forest_plots.py` | `output/jacr_revision/` |
| Figure S, external validation | `10_jacr_forest_plots.py` | `output/jacr_revision/` |
| SVI-EDI correlation | `09_validated_index_sdi.py`, `11_edi_tables_and_stats.py` | `validated_index_results.json`, `additional_statistics.json` |
| Every number, cross-checked | `12_manuscript_numbers.py` | `output/validation/` |

Figures 1B and S are drawn directly from `validated_index_results.json`, so a
...
| `publications/` | Manuscript drafts, kept out of version control while under review |
